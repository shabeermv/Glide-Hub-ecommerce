...
const productAdd = async (req, res) => {
  try {
    ...
    const totalStock = sizesWithStock.reduce(
...
const updateProduct = async (req, res) => {
  try {
    ...
    const parsedSizesWithStock =
      typeof sizesWithStock === "string"
        ? JSON.parse(sizesWithStock)
        : sizesWithStock;

    const parsedRemovedImages =
    ...
    const totalStock = parsedSizesWithStock.reduce(
      (sum, size) => sum + size.stock,
      0
    );
    ...
    if (parsedRemovedImages.length > 0) {
      ...
      updatedImages = updatedImages.filter(
        (img) => !parsedRemovedImages.includes(img)
      );
    }
